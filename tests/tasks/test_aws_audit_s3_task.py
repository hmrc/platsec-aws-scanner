...
from tests.test_types_generator import account, bucket, bucket_encryption, bucket_logging
...
class TestAwsAuditS3Task(AwsScannerTestCase):
    def test_run_task(self) -> None:
        ...
        self.assertEqual(
            {
                "buckets": [
                    bucket(bucket_1, bucket_encryption(enabled=True, type="cmk"), bucket_logging(enabled=False)),
                    bucket(bucket_2, bucket_encryption(enabled=False), bucket_logging(enabled=False)),
                    bucket(bucket_3, bucket_encryption(enabled=True, type="aws"), bucket_logging(enabled=True)),
                ]
            },
            task_report,
        )
