...
class TestAwsOrganizationsClient(TestCase):
    ...
    def test_get_target_accounts_includes_root(self) -> None:
        self.assertEqual(
            responses.EXPECTED_TARGET_ACCOUNTS, self.get_org_client().get_target_accounts("Root 1 > Org Unit 2")
        )

    def test_get_target_accounts_not_includes_root(self) -> None:
        with patch("src.aws_scanner_config.AwsScannerConfig.organization_include_root_accounts", return_value=False):
            self.assertEqual(
                responses.EXPECTED_TARGET_ACCOUNTS_WITHOUT_ROOT,
                self.get_org_client().get_target_accounts("Root 1 > Org Unit 2"),
            )
