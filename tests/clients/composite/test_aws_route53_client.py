...
from src.data.aws_logs_types import to_log_group, LogGroup
...
class TestAwsRoute53Client(TestCase):
        ...
        def test_enforcement_actions_with_existing_LogGroup(self) -> None:
                ...
                expectedLogGroups = [
                                        LogGroup(
                                            name = "logs_route53_log_group_name",
                ...
                expectedQueryLogActionList.append(DeleteQueryLogAction( route53_client= boto_route53, config= config, hosted_zone_id ="/hostedzone/AAAABBBBCCCCDD"))
                expectedQueryLogActionList.append(CreateQueryLogAction(account= account, route53_client= boto_route53, iam= iam, config= config, zone_id ="/hostedzone/AAAABBBBCCCCDD"))
                expectedQueryLogActionList.append(DeleteQueryLogAction( route53_client= boto_route53, config= config, hosted_zone_id ="/hostedzone/IIIIIIILLLLLLL"))
                expectedQueryLogActionList.append(CreateQueryLogAction(account= account, route53_client= boto_route53, iam= iam, config= config, zone_id= "/hostedzone/IIIIIIILLLLLLL"))
                client = AwsRoute53Client(boto_route53, iam, logs, kms, config)
