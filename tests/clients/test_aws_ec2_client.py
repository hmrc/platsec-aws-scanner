...
class TestAwsEC2ClientDescribeVpcs(AwsScannerTestCase):
    def test_describe_vpcs_empty(self) -> None:
        ...
        self.assertEqual([], ec2_client.describe_vpcs())
    ...
    def test_describe_vpcs(self) -> None:
        ...
        self.assertEqual([Vpc("vpc-12312e654bf654d12"), Vpc("vpc-984a4654b65465e12")], ec2_client.describe_vpcs())
...
class TestAwsEC2ClientDescribeFlowLogs(AwsScannerTestCase):
    ...
    def test_describe_flow_logs_empty(self) -> None:
        self.assertEqual([], self.ec2_client().describe_flow_logs(Vpc("vpc-no-flow-logs")))

    def test_describe_flow_logs(self) -> None:
        self.assertEqual(responses.EXPECTED_FLOW_LOGS, self.ec2_client().describe_flow_logs(Vpc("vpc-with-flow-logs")))

    def test_describe_flow_logs_failure(self) -> None:
        with redirect_stderr(StringIO()) as err:
            self.assertEqual([], self.ec2_client().describe_flow_logs(Vpc("vpc-error")))
        self.assertIn("AccessDenied", err.getvalue())
